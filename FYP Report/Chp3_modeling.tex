...
This chapter marks the first step in the formalization process by translating the core components of the Isolation Lemma into rigorous definitions within the Isabelle/HOL theorem proving environment.
...
This transition corresponds to the intermediate stage in the formalization pipeline:
from Pen-and-Paper Proofs (PP) to a fully formal representation in Isabelle/HOL, in which all objects are precisely defined and machine-readable, but where the main correctness results are not yet established.
...
This construction is designed so that the contribution of $x$ can be analyzed without directly depending on the sampled value of $w(x)$ itself, a point that becomes essential in the later probabilistic argument.
...
To bridge this gap, we make us of the \texttt{HOL-Probability} library \cite{isabelleprob}, which formalizes discrete distributions using measure theory.
